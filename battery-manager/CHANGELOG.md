...
- **Fix: Sell-window reserve floor** — Rolling schedule generation now reserves future sell windows above the highest configured floor from `soc.min_soc` and `soc.conservative_soc`, so 100% SOC does not treat the full battery as freely sellable.
