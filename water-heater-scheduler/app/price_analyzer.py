...
class PriceAnalyzer:
    ...
    def compare_today_vs_tomorrow(self, current_hour_start: time = time(6, 0)) -> Optional[bool]:
        """Compare if today's prices are cheaper than tomorrow's.
        
        Used for day program decisions.
        
        Args:
            current_hour_start: Start time for comparison window
            
        Returns:
            True if today is cheaper, False if tomorrow is cheaper, None if no data
        """
        now = datetime.now(self.timezone)
        
        # Get today's remaining prices
        today_prices = {dt: p for dt, p in self._price_curve.items() 
                       if dt.date() == now.date() and dt.time() >= current_hour_start}
        ...
        tomorrow_avg = sum(tomorrow_prices.values()) / len(tomorrow_prices)
