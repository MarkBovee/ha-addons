...
### Changed
- Refactored MQTT entity update flow by extracting reusable sensor update helpers.
- Consolidated repeated statistics sensor updates to reduce duplication and improve maintainability.
...
### Fixed
- Corrected "Max Profit" calculation to use `Max Export - Min Import` instead of `Max Import - Min Import`
- Updated MQTT sensor attributes for `max_profit_today` to show `min_import_price` and `max_export_price`

## [1.5.3] - 2026-01-01

### Added
- **Export Statistics**: Added new sensors for export price statistics: `sensor.energy_prices_average_export_price`, `sensor.energy_prices_minimum_export_price`, and `sensor.energy_prices_maximum_export_price`.

## [1.5.2] - 2026-01-01

### Fixed
- **Export Price Calculation**: Updated export price formula to include `markup` and `energy_tax` components, ensuring export prices reflect full netting (salderen) plus bonuses.
- **Configuration**: Added `export_markup` and `export_energy_tax` to configuration options.
